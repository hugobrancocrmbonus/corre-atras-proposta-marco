...
const VARIANT_GLOW: Record<'amber' | 'green', string> = {
  amber: 'rgba(255,187,64,0.08)',
  green: 'rgba(129,209,71,0.08)',
}

const VARIANT_BORDER: Record<'amber' | 'green', string> = {
  amber: 'linear-gradient(90deg, rgba(255,187,64,0.65), rgba(34,39,43,0.3)) border-box',
  green: 'linear-gradient(90deg, rgba(129,209,71,0.65), rgba(34,39,43,0.3)) border-box',
}

export function Macrocard({ title, subtitle, tagLabel, variant = 'amber', cards = [], children }: MacrocardProps) {
  return (
    <div
    ...
    >
      ...
      <motion.div
        ...
        style={{
          ...
          height: 576,
          borderRadius: '50%',
          background: VARIANT_GLOW[variant],
          filter: 'blur(80px)',
        }}
        animate={{
          top:  ['8%',  '8%',  '60%', '60%', '8%'],
      ...
      <div style={{ position: 'relative', zIndex: 1, padding: '32px 40px 40px 40px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 16 }}>
          ...
          {subtitle && (
            <p style={{ margin: 0, fontSize: 14, lineHeight: '20px', color: '#BCC6CD' }}>
              {subtitle}
            </p>
          )}
          {tagLabel && (
