...
type Page = 'agents' | 'create-campaign' | 'corre-atras' | 'consultor'
...
function breadcrumbFor(page: Page, nav: { toAgents: () => void; toCorreAtras: () => void }) {
  const home   = { label: 'Página inicial' }
  const agents = { label: 'Agentes IA', onClick: nav.toAgents }
  const corre  = { label: 'Corre Atrás', onClick: nav.toCorreAtras }

  switch (page) {
    case 'agents':          return [home, { label: 'Agentes IA' }]
    case 'corre-atras':     return [home, agents, { label: 'Corre Atrás' }]
...
export function App() {
  ...
  function renderPage() {
    switch (page) {
      case 'agents':
        return <AgentsPage onSelectAgent={() => setPage('corre-atras')} />
      case 'create-campaign':
        return (
          <CreateCampaignPage
            onCancel={() => setPage('corre-atras')}
